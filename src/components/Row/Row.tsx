...
export const Row: React.FC<RowType> = ({
    ...
    const [modeNotEdit, setModeNotEdit] = useState(modeNotEditArray);
    const [editIndex, setEditIndex] = useState<null | number>(null); // Индекс редактируемого элемента
    const [editValue, setEditValue] = useState('');

    // for list name row
    const [name, setName] = useState('Южная строительная площадка')
    ...
    const onChangeMode = (index: number) => {
        setEditIndex(index);
        setEditValue(modeNotEdit[index]);
    };
    ...
    const handleInputBlur = () => {
        ...
        newModeNotEdit[editIndex as number] = editValue;
                    ...
                    <li className={style.row__icon} style={{ paddingLeft: `${paddingLeft}px` }}>
                        <span
                            ...
                            {stateSvg && <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_3704_303)">
            ...
            <div className={style.row__item}>
                <ul className={style.row__item_list}>
                    {modeNotEdit.map((item, index) => {
                        return (
                            <li key={item} className={style.row__text}>
                                {editIndex === index ?
                                    <Input
                                        name={String(item)}
