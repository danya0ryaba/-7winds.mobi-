...
type RowTypeProps = { className?: string } & RowType

// const modeNotEditArray = ['20 348', '1 750', '108,07', '1 209 122,5']
...
export const Row: React.FC<RowTypeProps> = ({
    ...
    const handleInputBlur = () => {
        if (editIndex !== null) { // Проверяем, что editIndex не null
            const newModeNotEdit = [...modeNotEdit];
            newModeNotEdit[editIndex] = editValue; // Используем editIndex
            setModeNotEdit(newModeNotEdit);
            setEditIndex(null);
        }
    };

    const classForFirstElement = total === 0 ? style.first_element : '';
                ...
                <ul className={style.row__item_list}>
                    <li className={style.row__icon} style={{ paddingLeft: `${paddingLeft}px` }}>
                        <span
                            ...
                            {stateSvg && <svg
                                onClick={() => alert('delete current row?')}
                                width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
